...
public class FilmRepository implements IFilmRepo {
    ...
    public FilmRepository(@Value("${datafile}") String datafile) {
        try {
            ...
            ObjectMapper mapper =
                    new ObjectMapper(); // Dùng để ánh xạ cột trong CSV với từng trường trong POJO
    ...
    public Entry<String, Integer> getcountryMakeMostFilms() {
        ...
        return Map.entry(mapCountry.getKey(), mapCountry.getValue().intValue()); */
    }
    ...
    public List<String> getAllGeneres() {
        ...
        return films.stream().flatMap(film -> film.getGeneres().stream()).distinct().toList();
    }

    @Override
    // Tìm film do một quốc gia sản xuất từ năm X đến năm Y
    public List<Film> getFilmsMadeByCountryFromYearToYear(
            String country, int fromYear, int toYear) {
        return films.stream()
                .filter(
                        film ->
                                film.getCountry().equals(country)
    ...
    public Map<String, List<Film>> categorizeFilmByGenere() {
        ...
        for (String genere : getAllGeneres()) {
            var list =
                    films.stream()
                            .filter(film -> film.getGeneres().contains(genere))
    ...
    public List<Film> top5HighMarginFilms() {
        return films.stream()
                .sorted(
                        (f1, f2) ->
                                (f2.getRevenue() - f2.getCost()) - (f1.getRevenue() - f1.getCost()))
                .limit(5)
                .toList();
    }
    ...
    public double ratioBetweenGenere(String genreX, String genreY) {
        /* return categorizeFilmByGenere().get(genreX).size()
                * 1.0
                / categorizeFilmByGenere().get(genreY).size(); */
        
        return (double) categorizeFilmByGenere().get(genreX).size()
        / categorizeFilmByGenere().get(genreY).size();
    }
    ...
    public List<Film> top5FilmsHighRatingButLowMargin() {
        /* return films.stream()
                .sorted(
                        Comparator.comparing(Film::getRating)
                                .reversed()
                                .thenComparing(
                                        (f1, f2) ->
                                                -(f2.getRevenue() - f2.getCost())
                                                        + (f1.getRevenue() - f1.getCost())))
                .limit(5)
                .toList(); */
        ...
        .sorted(
                Comparator.comparing(Film::getRating)
                        .thenComparing(
                                (o1, o2) -> (o1.getRevenue() - o1.getCost())
                                        - (o2.getRevenue() - o2.getCost()))
                        .reversed())
        .limit(5)
        .toList();
    }
